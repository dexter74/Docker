...
````yaml
##########################################################################################################################
# Editeur - Interface de Développement WebUi (Version 3.1.5)
  Cloud9:
...
# Multimedia - Lecteur Multimédia
  Emby:
    ...
    volumes:
      ...
      - 'Emby:/data/tvshows'
      - '/mnt/DL:/data/DL:ro'
      - '/mnt/Musique:/data/music:ro'
      - '/mnt/Video:/data/movies:ro'
    labels:
       cacher: 'non'
##########################################################################################################################
````
...
````yaml
##########################################################################################################################
# Editeur - Interface de Développement WebUi (Version: 2.6.0)
  Pylon:
...
# Torrent - Client torrent
  QBitTorrent:
    ...
    volumes:
      - 'QBitTorrent:/config'
      - '/mnt/DL/QBitTorrent:/downloads:rw'
    labels:
       cacher: 'non'
#       
...
# echo "Downloads\SavePath=/mnt/DL/QBitTorrent" >> /home/docker/volumes/QBitTorrent/_data/qBittorrent/qBittorrent.conf ;
...
````yaml
##########################################################################################################################
# Warez - Radarr est un gestionnaire de collection de films pour les utilisateurs d’Usenet et de BitTorrent
  Radarr:
...
````yaml
##########################################################################################################################
# Raneto - est une plate-forme open source Knowledgebase qui utilise des fichiers Markdown
  Raneto:
    image: 'linuxserver/raneto'
    container_name: 'CN_Raneto'
    network_mode: 'bridge'
    restart: 'no'
    hostname: 'Raneto'
    domainname: 'labs'
    ...
    labels:
       cacher: 'non'
# Créer un fichier X:\Raneto\_data\content\readme.md  et aller sur http://localhost/readme
# Problème de permission (Conteneur lancer en docker et permission en root)
##########################################################################################################################
````

````yaml
##########################################################################################################################
# Outils - Bureau à distance (Login / Password: abc)
# Environnement : https://hub.docker.com/r/linuxserver/rdesktop
  rDesktop:
    image: 'linuxserver/rdesktop'
    container_name: 'CN_rDesktop'
    network_mode: 'bridge'
    restart: 'no'
    hostname: 'rDesktop'
    domainname: 'labs'
...
````yaml
##########################################################################################################################
# Outils - Remmina est un client de bureau distant (abc:abc http://localhost/?login=true)
  Remmina:
    image: 'linuxserver/remmina'
    container_name: 'CN_Remmina'
    network_mode: 'bridge'
    restart: 'no'
    hostname: 'Remmina'
    domainname: 'labs'
...
````yaml
##########################################################################################################################
# Outils - Resilio utilise le protocole BitTorrent pour synchroniser les fichiers et les dossiers entre vos appareils.
  Resilio-Sync:
    image: 'linuxserver/resilio-sync'
    container_name: 'CN_Resilio-Sync'
    network_mode: 'bridge'
    restart: 'no'
    ...
    ports:
      - '1065:8888'    # WebUi
      - '55555:55555'  # Port de Synchronisation
    volumes:
      - 'Resilio-Sync:/config'
      - 'Resilio-Sync:/sync'
      - '/mnt/DL:/downloads'
